...
import data_manager
import push_results
from nhits_engine import compute_nhits_scores
...
def main():
    ...
    macro_df = data_manager.prepare_macro(df)   # loaded for consistency; N-HiTS itself is univariate
    ...
    for universe_name, tickers in config.UNIVERSES.items():
        print(f"\n=== Universe: {universe_name} (N-HiTS Engine) ===")
        ...
        for win in config.WINDOWS:
            min_bars = (win + config.PRED_HORIZON + config.NHITS_LOOKBACK
                        + config.NHITS_BATCH_SIZE * 2 + 5)
    ...
    print(f"\n=== N-HiTS Engine complete: {tab1_path.name}, {tab2_path.name} ===")
